    ...
    <div className="relative min-h-screen overflow-hidden bg-gradient-hero text-background">
      ...
      <header className="relative z-10">
        <div className="container flex h-16 items-center justify-between">
          ...
          <Link to="/" className="hidden text-sm text-background/60 hover:text-background transition-colors sm:inline">
            ← Voltar ao site
          </Link>
        </div>
      </header>
        ...
        <div className="w-full max-w-md animate-fade-up">
          <div className="rounded-3xl border border-background/10 bg-background/[0.04] p-8 shadow-elevated backdrop-blur-xl sm:p-10">
            <div className="mb-8 text-center">
              ...
              <p className="mt-2 text-sm text-background/60">Acesse sua oficina e siga em frente.</p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-2">
                <Label htmlFor="email" className="text-background/80">E-mail</Label>
                <div className="relative">
                  <Mail className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-background/40" />
                  <Input id="email" type="email" autoComplete="email" placeholder="voce@oficina.com" value={email} onChange={(e) => setEmail(e.target.value)} required
                    className="h-12 border-background/10 bg-background/5 pl-10 text-background placeholder:text-background/30 focus-visible:border-primary/60 focus-visible:ring-primary/30" />
                </div>
              </div>

              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label htmlFor="password" className="text-background/80">Senha</Label>
                ...
                <div className="relative">
                  ...
                  <Input id="password" type={showPassword ? "text" : "password"} autoComplete="current-password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required
                    className="h-12 border-background/10 bg-background/5 pl-10 pr-10 text-background placeholder:text-background/30 focus-visible:border-primary/60 focus-visible:ring-primary/30" />
                  <button type="button" onClick={() => setShowPassword((s) => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-background/40 hover:text-background/80 transition-colors" aria-label={showPassword ? "Ocultar senha" : "Mostrar senha"}>
                    {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                  </button>
                </div>
              </div>

              <Button type="submit" variant="hero" size="lg" className="group w-full" disabled={submitting}>
                {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <>Entrar <ArrowRight className="transition-transform group-hover:translate-x-1" /></>}
              </Button>

              <div className="relative my-2 flex items-center">
                <div className="flex-1 border-t border-background/10" />
                <span className="px-3 text-[11px] uppercase tracking-wider text-background/40">ou</span>
                <div className="flex-1 border-t border-background/10" />
              </div>

              <Button type="button" variant="outline" size="lg" onClick={handleGoogle} disabled={submitting}
                className="w-full border-background/15 bg-background/5 text-background hover:bg-background/10 hover:text-background">
            ...
            <div className="mt-8 flex items-center justify-center gap-2 text-xs text-background/50">
